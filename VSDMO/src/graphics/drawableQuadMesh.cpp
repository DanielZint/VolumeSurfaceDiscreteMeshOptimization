...
namespace Graphics {
	...
	DrawableQuadMesh::DrawableQuadMesh(const glm::vec4& color)
		: DrawableMesh(color)
		, m_vboPos(nullptr)
		, m_vboNormal(nullptr)
		, m_iboFaces(nullptr)
	...
	DrawableQuadMesh::~DrawableQuadMesh() {
		//cout << "DrawableMesh Destructor" << endl;
	}

	void DrawableQuadMesh::init(int nVertices, int nFaces, int nElements) {
		...
		m_iboFaces->setNumElements((size_t)nFaces * 4);
		m_iboFaces->setSize((size_t)nFaces * 4 * sizeof(int));
	...
	void DrawableQuadMesh::initTbo(int nElements) {
	...
	void DrawableQuadMesh::render(const Graphics::Camera& camera) const {
		bindVao();
		GLuint shaderProg = Graphics::ShaderManager::getInstance().getProgramGL("quadsPhongLightingNormals");
		...
		glCall(glDrawElements(GL_LINES_ADJACENCY, (int)m_iboFaces->getNumElements(), GL_UNSIGNED_INT, 0));
		unbindVao();
	}

	void DrawableQuadMesh::renderFaces(const Graphics::Camera& camera, const glm::vec4& color) const {
		bindVao();
		GLuint shaderProg = Graphics::ShaderManager::getInstance().getProgramGL("quadsPhongLightingNormals");
	...
	void DrawableQuadMesh::renderEdges(const Graphics::Camera& camera, const glm::vec4& color) const {
		bindVao();
		GLuint shaderProg = Graphics::ShaderManager::getInstance().getProgramGL("quadsToEdges");
	...
	void DrawableQuadMesh::renderFacesAndEdges(const Graphics::Camera& camera, const glm::vec4& color1, const glm::vec4& color2) const {
	...
	void DrawableQuadMesh::renderVertex(const Graphics::Camera& camera, const glm::vec4& color) const {
		bindVao();
	...
	void DrawableQuadMesh::renderVertices(const Graphics::Camera& camera, int startOffset, int endOffset, const glm::vec4& color) const {
		glCall(glEnable(GL_POLYGON_OFFSET_FILL));
		bindVao();
	...
	void DrawableQuadMesh::renderElementQuality(const Graphics::Camera& camera, const glm::vec4& color) const {
		bindVao();

		GLuint shaderProg = Graphics::ShaderManager::getInstance().getProgramGL("quadQuality");
		//GLuint shaderProg = Graphics::ShaderManager::getInstance().getProgramGL("flatShading");
	...
	void DrawableQuadMesh::setVertexHighlighted(int vid) {
		m_iboHighlightVertex->bind();
		//glBufferData(m_iboHighlightVertex->getTarget(), m_iboHighlightVertex->getSize(), highlistVertexIndices, GL_DYNAMIC_DRAW);
	...
	DrawableInteropQuadMesh::DrawableInteropQuadMesh(const glm::vec4& color)
		: DrawableQuadMesh(color)
	{
	...
	DrawableInteropQuadMesh::~DrawableInteropQuadMesh() {
	...
	void DrawableInteropQuadMesh::init(int nVertices, int nTriangles, int nElements) {
		DrawableQuadMesh::init(nVertices, nTriangles, nElements);
	...
	void DrawableInteropQuadMesh::updatePositionsDev(void* points, size_t size) {
		interopPos->map();
	...
	void DrawableInteropQuadMesh::updateNormalsDev(void* normals, size_t size) {
		interopNormal->map();
	...
	void DrawableInteropQuadMesh::setIboDev(void* data, size_t size) {
		interopIbo->map();
	...
	void DrawableInteropQuadMesh::updateQualitiesDev(void* data) {
		interopTbo->map();
