...
namespace SurfLS {
	...
	__host__ void computeLocalSurfacesFeature(DMOMesh& mesh, device_vector<localSurface>& localSurfaces, device_vector<int>& neighbors, device_vector<int>& numNeighbors) {
		...
		gpuErrchk(cudaMalloc(&ATAData, nSurfaces * 9 * sizeof(float)));
		gpuErrchk(cudaMalloc(&ATbData, nSurfaces * 3 * sizeof(float)));
		gpuErrchk(cudaMalloc(&pivotArray, nSurfaces * 3 * sizeof(int)));
		...
		int lda = 3;
		checkcuBLASError(cublasSgetrfBatched(handle, 3, outMatrixPtrs, lda, pivotArray, infoArray, nSurfaces), "cublasSgetrfBatched() failed!");

		int info;
		checkcuBLASError(cublasSgetrsBatched(handle, CUBLAS_OP_N, 3, 1, outMatrixPtrs, lda, pivotArray, outRhsPtrs, lda, &info, nSurfaces), "cublasSgetrsBatched() failed!");
		if (info) {
