...
#include "DMOTetImplAtomic.h"
#include "DMOTetImplCub.h"
...
namespace DMO {
	...
	__global__ static void k_qualityHistogram(MeshTetDevice* mesh, ArrayView<int> q_vec, ArrayView<float> q_min, int n_cols, QualityCriterium q_crit) {
		for (int idx = threadIdx.x + blockIdx.x * blockDim.x; idx < mesh->nTetrahedra; idx += blockDim.x * gridDim.x) {
			const Tetrahedron& tet = mesh->tetrahedra[idx];
			const Vec3f points[4] = { mesh->vertexPoints[tet.v0], mesh->vertexPoints[tet.v1], mesh->vertexPoints[tet.v2], mesh->vertexPoints[tet.v3] };
			float q = qualityTet(points, q_crit);
	...
	__global__ static void k_findMinimumQuality(MeshTetDevice* mesh, ArrayView<float> q_min, QualityCriterium q_crit) {
		for (int idx = threadIdx.x + blockIdx.x * blockDim.x; idx < mesh->nTetrahedra; idx += blockDim.x * gridDim.x) {
			const Tetrahedron& tet = mesh->tetrahedra[idx];
			const Vec3f points[4] = { mesh->vertexPoints[tet.v0], mesh->vertexPoints[tet.v1], mesh->vertexPoints[tet.v2], mesh->vertexPoints[tet.v3] };
			float q = qualityTet(points, q_crit);
	...
	__global__ static void k_updateCurrentQualities(MeshTetDevice* mesh, ArrayView<float> currentQualities, float lastMinQuality, ArrayView<bool> failure, QualityCriterium q_crit) {
		for (int idx = threadIdx.x + blockIdx.x * blockDim.x; idx < mesh->nTetrahedra; idx += blockDim.x * gridDim.x) {
			const Tetrahedron& tet = mesh->tetrahedra[idx];
			const Vec3f points[4] = { mesh->vertexPoints[tet.v0], mesh->vertexPoints[tet.v1], mesh->vertexPoints[tet.v2], mesh->vertexPoints[tet.v3] };
			float q = qualityTet(points, q_crit);
			if (q < lastMinQuality) {
				failure[0] = true;
				printf("minimum quality decreased! tri %i vertices %i %i %i %i q %f\n", idx, tet.v0, tet.v1, tet.v2, tet.v3, q);
				assert(0);
			}
	...
	__global__ static void k_getElementQualities(MeshTetDevice* mesh, float* outBuffer, QualityCriterium q_crit) {
		for (int idx = threadIdx.x + blockIdx.x * blockDim.x; idx < mesh->nTetrahedra; idx += blockDim.x * gridDim.x) {
			const Tetrahedron& tet = mesh->tetrahedra[idx];
			const Vec3f points[4] = { mesh->vertexPoints[tet.v0], mesh->vertexPoints[tet.v1], mesh->vertexPoints[tet.v2], mesh->vertexPoints[tet.v3] };
			outBuffer[idx] = qualityTet(points, q_crit);
		}
	}
	...
	DMOTetClass::DMOTetClass(DMOMeshTet& dmo_mesh_, QualityCriterium qualityCriterium_, const float gridScale_, int n_iter_)
	...
	void DMOTetClass::init() {
		...
		currentQualities = device_vector<float>(dmo_mesh.nTetrahedra, -FLT_MAX);

		cout << "0," << lastMinQuality << endl;
	}


	void DMOTetClass::doIteration() {
		int dynMemSize3D = dmo_mesh.maxNumTetrahedra * 3 * 3 * sizeof(float);
		int dynMemSize2D = dmo_mesh.maxNumTetrahedra * 3 * 3 * sizeof(float) + dmo_mesh.maxNumHalfedges * sizeof(int);
		int dynMemSize1D = dmo_mesh.maxNumTetrahedra * 3 * 3 * sizeof(float) + dmo_mesh.maxNumHalfedges * sizeof(int);

		for (int cid = 0; cid < dmo_mesh.nColorsInner; ++cid) {
			k_optimizeHierarchicalInnerTet<128> << <dmo_mesh.colorOffsetsInner[cid + 1] - dmo_mesh.colorOffsetsInner[cid], 128, dynMemSize3D >> >
		...
		if (lastMinQuality > 0.f) {
			for (int cid = 0; cid < dmo_mesh.nColorsFree; ++cid) {
				k_optimizeHierarchical2DTet<DMO_NQ * DMO_NQ/2, USE_SURF_OF_NN> << <dmo_mesh.colorOffsetsFree[cid + 1] - dmo_mesh.colorOffsetsFree[cid], DMO_NQ* DMO_NQ/2, dynMemSize2D >> >
					(dmo_mesh.colorOffsetsFree[cid], dmo_mesh.colorOffsetsFree[cid + 1], dmo_mesh.d_mesh, affineFactor, qualityCriterium, gridScale,
						...
						raw(table), raw(surfacesRowPtr), raw(localSurfacesFeatureInit), dmo_mesh.maxNumTetrahedra * 3);
				gpuErrchk(cudaPeekAtLastError());
				gpuErrchk(cudaDeviceSynchronize());
			}

			for (int cid = 0; cid < dmo_mesh.nColorsFeature; ++cid) {
				k_optimizeHierarchical1DTet<DMO_NQ> << <dmo_mesh.colorOffsetsFeature[cid + 1] - dmo_mesh.colorOffsetsFeature[cid], DMO_NQ, dynMemSize1D >> >
					(dmo_mesh.colorOffsetsFeature[cid], dmo_mesh.colorOffsetsFeature[cid + 1], dmo_mesh.d_mesh, affineFactor, qualityCriterium, gridScale,
						...
						raw_pointer_cast(optimizeFeatureVertex.data()), dmo_mesh.maxNumTetrahedra * 3, dmo_mesh.maxNumHalfedges);
				gpuErrchk(cudaPeekAtLastError());
				gpuErrchk(cudaDeviceSynchronize());
			}
		}
	...
	void DMOTetClass::getEstimateLocalSurfacePoints(int vid, int nu, int nv, void* outSurfacePoints) {
	...
	void DMOTetClass::getLocalSurfacePoints(int vid, int nu, int nv, void* outSurfacePoints, int featureSid) {
	...
	void DMOTetClass::displayQualityGPU(int n_cols) {
		device_vector<int> q_vec(n_cols, 0);
		device_vector<float> q_min(1, FLT_MAX);
		const int BLOCK_SIZE = 128;
		k_qualityHistogram << <getBlockCount(dmo_mesh.nTetrahedra, BLOCK_SIZE), BLOCK_SIZE >> > (dmo_mesh.d_mesh, q_vec, q_min, n_cols, qualityCriterium);
		gpuErrchk(cudaPeekAtLastError());
		gpuErrchk(cudaDeviceSynchronize());
	...
	void DMOTetClass::getQualityHistogram(std::vector<int>& vec, int n_cols) {
		device_vector<int> q_vec(n_cols, 0);
		device_vector<float> q_min(1, FLT_MAX);
		const int BLOCK_SIZE = 128;
		k_qualityHistogram << <getBlockCount(dmo_mesh.nTetrahedra, BLOCK_SIZE), BLOCK_SIZE >> > (dmo_mesh.d_mesh, q_vec, q_min, n_cols, qualityCriterium);
	...
	float DMOTetClass::findMinimumQuality() const {
		device_vector<float> q_min(1, FLT_MAX);
		const int BLOCK_SIZE = 128;
		k_findMinimumQuality << <getBlockCount(dmo_mesh.nTetrahedra, BLOCK_SIZE), BLOCK_SIZE >> > (dmo_mesh.d_mesh, q_min, qualityCriterium);
		gpuErrchk(cudaPeekAtLastError());
		gpuErrchk(cudaDeviceSynchronize());
		return q_min[0];
	}

	void DMOTetClass::updateCurrentQualities() {
		const int BLOCK_SIZE = 128;
		device_vector<bool> failure(1, false);
		k_updateCurrentQualities << <getBlockCount(dmo_mesh.nTetrahedra, BLOCK_SIZE), BLOCK_SIZE >> > (dmo_mesh.d_mesh, currentQualities, lastMinQuality, failure, qualityCriterium);
		gpuErrchk(cudaPeekAtLastError());
		gpuErrchk(cudaDeviceSynchronize());
		if (failure[0]) {
			writeTetgen("res/fail_debug", dmo_mesh);
			throw 1;
		}
	}

	void DMOTetClass::getElementQualities(void* outFloatBuffer) {
		const int BLOCK_SIZE = 128;
		k_getElementQualities << <getBlockCount(dmo_mesh.nTetrahedra, BLOCK_SIZE), BLOCK_SIZE >> > (dmo_mesh.d_mesh, (float*)outFloatBuffer, qualityCriterium);
		gpuErrchk(cudaPeekAtLastError());
		gpuErrchk(cudaDeviceSynchronize());
	}
