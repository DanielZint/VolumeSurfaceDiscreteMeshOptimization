...
namespace DMOImplCub {
	...
	__global__ void k_optimizeHierarchical2D(int cStart, int cEnd, MeshQuadDevice* mesh, float affineFactor, QualityCriterium q_crit, float grid_scale,
		Vec3f* vertexPointsInit, localSurface* localSurfacesInit, int* nearestNeighbors, int* table, int* surfacesRowPtr, localSurface* localSurfacesFeatureInit, int offsetOneRing)
	{
		...
		int* s_surfVh = (int*)&s_oneRing[offsetOneRing];
		...
		for (int i = threadIdx.x; i < nHalfedgesRing; i += blockDim.x) {
			...
			s_oneRing[2 * i + 0] = oneRingPos;

			const Halfedge& halfedgeNext = mesh->halfedges.values_[halfedge.nextEdge];
			const int vdst2 = halfedgeNext.targetVertex;
			Vec3f oneRingPos2 = mesh->vertexPoints[vdst2];
			s_oneRing[2 * i + 1] = oneRingPos2;

			float d = (oneRingPos - s_currPos).norm();
			tlocalMin = fminf(tlocalMin, d);
			tlocalMax = fmaxf(tlocalMax, d);

			float d2 = (oneRingPos2 - s_currPos).norm();
			tlocalMin = fminf(tlocalMin, d2);
			tlocalMax = fmaxf(tlocalMax, d2); //differences here
		}
		...
		if (threadIdx.x == 0) {
			//s_minDist = tblockMin;
			s_maxDist = tblockMax;
			s_oneRing[2 * nHalfedgesRing] = s_oneRing[0];
			s_nOneRing = nHalfedgesRing; // 2 * nHalfedgesRing + 1;
		...
		for (int depth = 0; depth < DMO_DEPTH; ++depth) {
			...
			float qLast = qualityQuadRing(s_nOneRing, s_oneRing, s_currPos, mesh->vertexNormals[nnvid], q_crit);
			float q1 = qualityQuadRing(s_nOneRing, s_oneRing, pWorld1, mesh->vertexNormals[nnvid], q_crit);
			float q2 = qualityQuadRing(s_nOneRing, s_oneRing, pWorld2, mesh->vertexNormals[nnvid], q_crit);
		...
		float qOld = qualityQuadRing(s_nOneRing, s_oneRing, oldPos, q_crit);
	...
	__global__ void k_optimizeHierarchical1D(int cStart, int cEnd, MeshQuadDevice* mesh, float affineFactor, QualityCriterium q_crit, float grid_scale,
		Vec3f* vertexPointsInit, localSurface1d* localSurfacesInit1d, int* nearestNeighbors, bool* optimizeFeatureVertex, int offsetOneRing, int maxSurfRingSize)
	{
		if (blockIdx.x >= cEnd - cStart) {
			return;
		}
		...
		for (int i = threadIdx.x; i < nHalfedgesRing; i += blockDim.x) {
			...
			float xDist2 = abs(s_currPos.x - oneRingPos2.x);
			float yDist2 = abs(s_currPos.y - oneRingPos2.y);
			float zDist2 = abs(s_currPos.z - oneRingPos2.z);
			float d2 = std::sqrtf(xDist2 * xDist2 + yDist2 * yDist2 + zDist2 * zDist2);
			tlocalMin = fminf(tlocalMin, d2);
			tlocalMax = fmaxf(tlocalMax, d2);
		...
		if (threadIdx.x == 0) {
			...
			//s_minDist = tblockMin;
			s_maxDist = tblockMax;
			s_nOneRing = nHalfedgesRing - 1; // 2 * nHalfedgesRing;
		...
		for (int depth = 0; depth < DMO_DEPTH; ++depth) {
			...
			qMax = qualityQuadRing(s_nOneRing, s_oneRing, pWorld, mesh->vertexNormals[nnvid], q_crit);
