...
namespace DMOImplCub {
	...
	__global__ void k_optimizeHierarchicalInnerHex(int cStart, int cEnd, MeshHexDevice* mesh, float affineFactor, QualityCriterium q_crit, const float grid_scale)
	{
		...
		const int hfStart = mesh->halfhexes.rowPtr_[vid];
		const int nHalfhexes = mesh->halfhexes.rowPtr_[vid + 1] - hfStart;

		if (threadIdx.x == 0) {
			s_currPos = mesh->vertexPoints[vid];
			s_nOneRing = nHalfhexes;
		}
		...
		for (int i = threadIdx.x; i < nHalfhexes; i += blockDim.x) {
			const Halfhex& halfhex = mesh->halfhexes.values_[hfStart + i];
			Vec3f posRing[7] = { mesh->vertexPoints[halfhex.v0], mesh->vertexPoints[halfhex.v1], mesh->vertexPoints[halfhex.v2], mesh->vertexPoints[halfhex.v3], mesh->vertexPoints[halfhex.v4], mesh->vertexPoints[halfhex.v5], mesh->vertexPoints[halfhex.v6] };
			for (int j = 0; j < 7; ++j) { // for each point in halfface
				s_oneRing[7 * i + j] = posRing[j];
		...
		for (int depth = 0; depth < DMO_DEPTH; ++depth) {
			Vec3f gridMax = s_currPos + depth_scale * s_minDist;
			Vec3f gridMin = s_currPos - depth_scale * s_minDist;

			float qLast = qualityHexRing(s_nOneRing, s_oneRing, s_currPos, q_crit);
			...
			for (int idx = threadIdx.x * pointsPerThread; idx < (threadIdx.x + 1) * pointsPerThread; ++idx) {
				...
				float q = qualityHexRing(s_nOneRing, s_oneRing, p, q_crit);
				if (q > qMax) {
					qMax = q;
					pMax = p;
				}
		...
		float qOld = qualityHexRing(s_nOneRing, s_oneRing, oldPos, q_crit);
	...
	__global__ void k_optimizeHierarchical2DHex(int cStart, int cEnd, MeshHexDevice* mesh, float affineFactor, QualityCriterium q_crit, float grid_scale,
		Vec3f* vertexPointsInit, localSurface* localSurfacesInit, int* nearestNeighbors, int* table, int* surfacesRowPtr, localSurface* localSurfacesFeatureInit, int maxOneRingSize)
	{
		...
		const int hfStart = mesh->halfhexes.rowPtr_[vid];
		const int nHalfhexes = mesh->halfhexes.rowPtr_[vid + 1] - mesh->halfhexes.rowPtr_[vid];
		...
		for (int depth = 0; depth < DMO_DEPTH; ++depth) {
			...
			float q1 = qualityHexRing(s_nOneRing, s_oneRing, pWorld1, q_crit);
			float q2 = qualityHexRing(s_nOneRing, s_oneRing, pWorld2, q_crit);
	...
	template<int BLOCK_SIZE>
	__global__ void k_optimizeHierarchical1DHex(int cStart, int cEnd, MeshHexDevice* mesh, float affineFactor, QualityCriterium q_crit, float grid_scale,
		Vec3f* vertexPointsInit, localSurface1d* localSurfacesInit1d, int* nearestNeighbors, bool* optimizeFeatureVertex, int maxOneRingSize, int maxSurfRingSize)
	{
		...
		for (int depth = 0; depth < DMO_DEPTH; ++depth) {
			...
			qMax = qualityHexRing(s_nOneRing, s_oneRing, pWorld, q_crit);
